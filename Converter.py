...
%put NOTE: Dataset successfully imported from XPORT file;
%put NOTE: Saved as outlib.{file_name} in current directory;
"""
        else:
            # Generate CSV import script (original logic)
...
libname outlib ".";
data outlib.{file_name};
    set {file_name}_typed;
run;

/* Print summary */
proc contents data={file_name}_typed;
run;

proc print data={file_name}_typed (obs=10);
run;
"""
        return sas_code
            ...
            with h5py.File(h5_path, 'r') as f:
                ...
                for idx, (name, dataset) in enumerate(datasets_to_convert):
                    ...
                    try:
                        ...
                        # Also create a SAS import script for the XPORT file
                        sas_script_path = xport_path.with_suffix('.sas')
                        sas_import_code = self.generate_sas_import_script(xport_path, df, file_type='xport')
                        with open(sas_script_path, 'w') as f:
                            f.write(sas_import_code)
                        logger.info(f"  Created SAS import script for XPORT: {sas_script_path}")
                    ...
                    if not saved_successfully:
                        ...
                        # Also create a SAS import script for CSV
                        ...
                        with open(sas_script_path, 'w') as f:
                            f.write(sas_import_code)
                        logger.info(f"  Created SAS import script for CSV: {sas_script_path}")
                        saved_successfully = True
